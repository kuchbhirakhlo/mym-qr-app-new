...
import { auth, db, GoogleAuthProvider, doc, safeCreateUserWithEmailAndPassword, safeSetDoc, safeSignInWithPopup } from "@/lib/firebase"
...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log("Creating user with:", { email, restaurantName })

      // Create user with email and password
      const userCredential = await safeCreateUserWithEmailAndPassword(auth, email, password)
      const user = userCredential.user

      console.log("User created:", user.uid)

      // Create user document in Firestore
      await safeSetDoc(doc(db, "restaurants", user.uid), {
        restaurantName,
        email,
        createdAt: new Date().toISOString(),
      })
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      // Use a try/finally block to ensure isLoading is reset
      try {
        ...
        if (result.user) {
          ...
          // Force a redirect to dashboard using window.location for more reliability
      ...
      if (error.code === 'auth/configuration-not-found') {
        errorMessage = "Google authentication is not properly configured. Please try email signup instead.";
      } else if (error.code === 'auth/popup-closed-by-user') {
        errorMessage = "You closed the popup before completing authentication. Please try again.";
      } else if (error.code === 'auth/popup-blocked') {
        errorMessage = "The sign-in popup was blocked by your browser. Please allow popups for this site.";
      } else if (error.code === 'auth/cancelled-popup-request') {
        errorMessage = "Authentication popup was cancelled. Please try again and avoid clicking multiple times.";
      } else if (error.message) {
        errorMessage = error.message;
      }
      ...
      <Card className="w-full max-w-md border-orange-200 shadow-lg bg-white">
        <CardHeader className="space-y-1 bg-gradient-to-r from-orange-500 to-amber-500 rounded-t-lg">
          ...
          <CardDescription className="text-orange-100">
            Sign up to create your restaurant menu and QR code
          </CardDescription>
        </CardHeader>
